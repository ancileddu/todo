...
function check_status(){
  if($('#all_check').is( ':checked' )){
    ...
    if ($('#closed_check').is( ':checked' )){
      //$('#opened_check').attr('checked', false);
        $('#opened_check').prop('checked', false);
   ...
   // }
      ...
      if ($('#opened_check').is( ':checked' )){
        ...
        //$('#closed_check').attr('checked', false);
...
$('select#view, #opened_check, #closed_check, #all_check').on('change', function() {
  ...
  //if ($('#open_check').prop('checked'))   alert($('#open_check').val());
if($('select#view').val() == "i tuoi task"){
  //alert("contenuto completed: " + completed.toString());
deleted_empty(deleted);
...
check_status();
	...
	if ($('select#view').val() == "visualizza tutti"){
    deleted_empty(deleted);
    check_status();

	}
else 
	if ($('select#view').val() == "task assegnati"){
    deleted_empty(deleted);
